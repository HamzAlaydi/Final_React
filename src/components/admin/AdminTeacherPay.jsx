...
export default function AdminTeacherPay({id}) {
    ...
    async function onSubmit(data)
    {
        ...
        setLoad(true)
        try{
            const response = await fetch(`${process.env.REACT_APP_API_KEY}api/v1/admin/pay`,{
                headers:{
                    "Authorization":token,
            ...
            {
                enqueueSnackbar(lang==="ar"?resData.msg.arabic:resData.msg.english,{variant:"error",autoHideDuration:"8000"})
    ...
    return (
        <Box>
            ...
            <Box sx={{marginBottom:"18px"}}>
                <InputLabel sx={{marginBottom:"10px",fontSize:"14px"}}>{t('amount')}</InputLabel>
                <Controller
                name="amount"
                control={control}
            ...
            {
                !load?
                <Button type="submit" variant="contained">{t('pay')}</Button>
                :
