...
import { DialogContent,DialogActions,InputLabel,Button, Box,TextField,Typography, FormLabel } from '@mui/material'
...
export default function UpdateSubject({handleClose,subject,setSubjects}) {
    ...
    async function onSubmit(data)
    {
        ...
        try{
            const response = await fetch(`${process.env.REACT_APP_API_KEY}api/v1/admin/updateSubCategories/${subject.id}`,{
                method:"PUT",
                headers:{
                    "Authorization":token,
                },
            ...
            console.log(resData);
            setSubjects(back=>back.map(item=>
                {
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <DialogContent>
                    <Box sx={{marginBottom:"8px" }}>
                        <FormLabel htmlFor='image' sx={{ cursor:"pointer"}}>تعديل الصورة</FormLabel>
