...
function Navbar(props) {
    ...
    React.useEffect(() => {
        if(teacher){
            const q = query(collection(db,"Notifications"),where("TeacherId", "==", `${teacher.id}`));
    ...
    const teacherProfile = [{title:t('profile'),link:"/about",icon:<ManageAccountsIcon fontSize="small"/>},
    {title:t('lessons'),link:"/lessons",icon:<CastForEducationIcon fontSize='small'/>},{title:t('messages'),link:"/messages",icon:<EmailIcon fontSize='small'/>},
    ...
    const drawer = (
        <Box onClick={handleDrawerToggle} sx={{ textAlign: 'center' , height:"100vh" , backgroundColor:"#005B8E"}}>
        ...
        <List>
            <ListItem  disablePadding>
                ...
                <a target='_black' href="https://api.whatsapp.com/send?phone=96894414423">
            ...
            <SelectCurrency/>
            {
                ...
                <Link to='/teacher/notifications'>
                    <ListItem sx={{color:"white",display:"flex",justifyContent:"center"}}>{t('notifications')}</ListItem>
                </Link>
                {
                    teacherProfile.map((item,index)=>
                    {
                        return(
                            <Link to={`/teacher${item.link}`} key={index+'a1'}>
                                <ListItem sx={{color:"white",display:"flex",justifyContent:"center"}}>{item.title}</ListItem>
                            </Link>
                        )
                    })
                }
            ...
            }
            {
                ...
                <Link to="/student/profile">
                    <ListItem sx={{color:"white",display:"flex",justifyContent:"center"}}>{t('profile')}</ListItem>
                </Link>
            ...
            {!teacher&&!parent&&!student&&
                <Box sx={{display:"flex",flexDirection:"column",alignItems:"center"}}>
                    <Button sx={{ my: 1, color: 'white', display: 'block',textTransform:"capitalize",
                    padding:"1px 18px"}} variant="text" onClick={()=>navigate('/login')}>{t('login')}</Button>
                    <Button onClick={()=>navigate('/teacherRegister/step1')}
                    sx={{ color: 'white', display: 'block',textTransform:"capitalize",
    ...
    <Box sx={{ display: 'flex' }}>
        <AppBar component="nav">
            ...
            <Box sx={{ display: { xs: 'none', md: 'flex' },alignItems:"center",columnGap:"10px" }}>
                ...
                }
                {
                    ...
                    <Stack alignItems={"center"} direction="row" gap="12px">
                        <Box sx={{width:"35px",height:"35px",backgroundColor:"#fc5a5a",display:"flex",
                        justifyContent:"center",alignItems:"center",borderRadius:"50%",cursor:"pointer"}}
                        ...
                        <Box>
                            ...
                            onClick={handleClick}
                            >
                                <Box sx={{width:"35px",height:"35px",backgroundColor:"#ffffff33",display:"flex",justifyContent:"center",alignItems:"center",borderRadius:"50%"}}><PersonIcon sx={{fontSize:"22px"}}/></Box>
                                {teacher?.firstName?teacher?.firstName+" "+teacher?.lastName:t('username')}
                            </Box>
                            <Menu
                                id="account-menu"
                                anchorEl={anchorEl}
                            ...
                            >
                                {
                                    ...
                                    {
                                        return <MenuItem sx={{fontSize:"14px"}} onClick={()=>{navigate(`/teacher${item.link}`);handleClose();}}>
                                            <ListItemIcon>
                                                {item.icon}
                                            </ListItemIcon>
                                            {item.title}
                                ...
                                <MenuItem sx={{fontSize:"14px"}} onClick={handleTeacherLogout}>
                                    <ListItemIcon>
                ...
                }
                {
                    ...
                    <Stack direction="row" alignItems={"center"}>
                        <Button color="Blue" variant="contained" sx={{mx:"8px"}} onClick={()=>navigate('/student/profile')}>{student?.name?student.name:t('username')}</Button>
            ...
            <Box sx={{ display: { xs: 'flex', md: 'none' },alignItems:"center",columnGap:"10px" }}>
            <Link to="/landing">
                <Box sx={{display:"flex",alignItems:"center" , justifyContent:"center",color:"white" , marginBottom:"12px"}}>
                    <SearchIcon/>
                </Box>
            </Link>
            <ChangeLanguage lang={lang}/>
