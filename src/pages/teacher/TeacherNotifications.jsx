...
export default function TeacherNotifications() {
    ...
    useEffect(() => {
        async function updateNotification(){
            try{
                const response = await fetch(`${process.env.REACT_APP_API_KEY}api/v1/teacher/updateNotification/${teacher.id}`,{
                    headers:{
                        "Authorization":token,
    ...
    return (
        <Navbar>
            <Container sx={{marginBottom:"60px",marginTop:"120px",padding:"40px 20px 20px"}} component={Paper}>
                {
                    notifications.length>0 && notifications.map(not=>{
                        return <Wrapper sx={{marginBottom:"35px"}} key={not.id}>
                        <IconWrapper sx={{backgroundColor:!not.seen?"#e66b4c47":"#40c0dc33"}}>
                        ...
                        <Box sx={{width:"90%"}}>
                            ...
                            <Typography sx={{fontSize:"13px",color:"#949494",fontWeight:"500"}}>
