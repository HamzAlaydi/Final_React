...
export default function SingleTeacher() {
    ...
    const handleCreateMessage = async () =>{
        ...
        const q = query(collection(db, "chats"), where("studentId", "==", `${student.id}`), where("teacherId", "==", `${id}`));
        const res = await getDocs(q);
        if(res.empty){
            const time = Timestamp.now();
            await addDoc(collection(db, "chats"), { messages: [] , teacherId:`${id}` ,studentId:`${student.id}`,
            studentName:student.name , studentImage:student.image , teacherName:`${data?.data?.firstName} ${data?.data?.lastName}`,
            teacherImage:data?.data?.image,lastmessage:time });
        }
                ...
                <Grid container spacing={3}>
                    ...
                    <Grid item md={12} lg={4}>
                        <Paper sx={{padding:"24px 12px",marginY:"30px"}}>
                            ...
                            sx={{textTransform:"capitalize",marginBottom:"16px"}} onClick={handleRequestBook}>{t('requestBook')}</Button>
                            <Button variant="outlined" fullWidth
                            sx={{textTransform:"capitalize"}} onClick={handleCreateMessage}>{t('contactTeacher')}</Button>
                        </Paper>
                    </Grid>
                </Grid>
            </Container>
